...
export default function Preview206() {
...
<Toolbar toggleStyle={toggleStyle} changeAlignment={changeAlignment} toggleList={toggleList}  /> 
            ...
            spellCheck={true}
                ...
                <div className='header205'>
                    <div className="left205" style={{marginBottom:'-17px',paddingTop: '10px',paddingBottom:'10px',padding:'5px'}}>
                        <h3 className='ln-.5' style={{ ...dynamicStyle.heading,marginBottom:'-8px'}}>{result.fullname}</h3>
                        <small className='color-#5bb7ce' style={{...dynamicStyle,marginBottom:'10px',color:'#5bb7ce'}}>{result.jobposition}</small>
                    </div>

                    <div className="right205 p-2 mt-4 mb-2 d-flex justify-content-around align-items-center" 
                    style={{backgroundColor:'#5bb7ce',borderRadius:'25px'}}>
                        <p className='text-end ' style={dynamicStyle}>{result.email} <span className="icon" style={{fontSize:fontSize}}>✉️</span></p>
                        <p className='text-end' style={dynamicStyle}>{result.mobileNumber} <span className="icon" style={{fontSize:fontSize}}>📞</span></p>
                        <p className='text-end' style={dynamicStyle}>{result.city}, {result.state || "CA"} <span className="icon" style={{fontSize:fontSize}}>📍</span></p>
                        <p className='text-end' style={dynamicStyle}>{result.linkedin} <span className="icon" style={{fontSize:fontSize}}>🔗</span></p>
                    </div>
                </div>
